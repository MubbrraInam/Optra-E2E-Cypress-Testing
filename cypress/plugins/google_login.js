...
module.exports.GoogleSocialLogin = async function GoogleSocialLogin(options = {}) {
  ...
  await browser.on('targetcreated', async (target) => {
    ...
    await targetPage.type('#password input[type="password"]', options.password, {delay: 50})
    await targetPage.click('#passwordNext')
    }
    
    })

  const token_val = await getToken({page, options})
...
async function getToken({page, options} = {}) {
